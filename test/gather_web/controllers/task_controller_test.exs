defmodule GatherWeb.TaskControllerTest do
  use GatherWeb.ConnCase

  alias Gather.Tasks

  @create_attrs %{completed: true, task: "some task", user: 42}
  @update_attrs %{completed: false, task: "some updated task", user: 43}
  ...
  describe "update task" do
    ...
    test "redirects when data is valid", %{conn: conn, task: task} do
      ...
      assert html_response(conn, 200) =~ "some updated task"
    end
